...
def build_curve(initial_mg: float, total_hours: int = 10):
    x = np.linspace(0, total_hours, 400)

    rise = 1 / (1 + np.exp(-4 * (x - 1.1)))
    decay = np.exp(-0.17 * np.clip(x - 2, 0, None))
    y = rise * decay

    scale = max(0.8, min(initial_mg / 120, 2.0))
    y = y * scale

    return x, y


def add_vertical_gradient(ax, x0, x1, y0, y1, color_rgb, alpha_top=0.55, alpha_bottom=0.20, zorder=0):
    n = 256
...
def draw_legend_like_bar():
    ...
    <div style="display:flex; justify-content:center; margin-top:10px; margin-bottom:10px;">
        ...
        ">
            <div style="flex:1; background:#dce6c8; padding:8px 0; text-align:center; font-size:13px; color:#1f1f1f;">Start</div>
            <div style="flex:1; background:#76bb2d; padding:8px 0; text-align:center; font-size:13px; color:white;">Increase</div>
            <div style="flex:1; background:#ff4338; padding:8px 0; text-align:center; font-size:13px; color:white;">Peak</div>
            <div style="flex:1; background:#ff9d35; padding:8px 0; text-align:center; font-size:13px; color:white;">Crash</div>
            <div style="flex:1; background:#7aa8cf; padding:8px 0; text-align:center; font-size:13px; color:white;">Recovery</div>
        </div>
    </div>
    """
...
def draw_recommendation_chart(initial_mg: float, hours_passed: float):
    ...
    # background gradients like in image
    add_vertical_gradient(ax, 0, 2, 0, y_max, (183/255, 223/255, 114/255), alpha_top=0.55, alpha_bottom=0.20)
    add_vertical_gradient(ax, 2, 4, 0, y_max, (190/255, 214/255, 84/255), alpha_top=0.55, alpha_bottom=0.20)
    add_vertical_gradient(ax, 4, 6, 0, y_max, (1.0, 84/255, 78/255), alpha_top=0.65, alpha_bottom=0.25)
    add_vertical_gradient(ax, 6, 8, 0, y_max, (243/255, 176/255, 104/255), alpha_top=0.50, alpha_bottom=0.20)
    add_vertical_gradient(ax, 8, 10, 0, y_max, (163/255, 194/255, 223/255), alpha_top=0.55, alpha_bottom=0.20)

    # phase borders
    for border_x in [2, 4, 6, 8]:
        ax.axvline(border_x, color=(0, 0, 0, 0.06), linewidth=1)

    # line
    ax.plot(x, y, color="#0D6AAD", linewidth=2.5, zorder=5)

    # current point
    current_x = min(hours_passed, 10)
    current_y = np.interp(current_x, x, y)

    ax.scatter(current_x, current_y, s=110, color="#0D6AAD", zorder=6)
    ax.text(
        current_x + 0.03,
        current_y + 0.075,
        "You are here",
...
def show_phase_help():
    ...
    with st.expander("Start"):
        st.write(
            "Caffeine has just entered your body. "
            "You usually do not feel the full effect yet."
        )

    with st.expander("Increase"):
        st.write(
            "The caffeine effect is getting stronger. "
            "You may start to feel more awake, focused or active."
        )

    with st.expander("Peak"):
        st.write(
            "This is the strongest phase. "
            "The caffeine effect is highest now."
        )

    with st.expander("Crash"):
        st.write(
            "The caffeine effect starts to go down again. "
            "Some people feel more tired, unfocused or low in energy here."
        )

    with st.expander("Recovery"):
        st.write(
            "Your body is slowly calming down again. "
            "The caffeine level is lower and the effect becomes weaker."
        )


# -----------------------------
# USER FEELING DETAIL PAGES
# -----------------------------
