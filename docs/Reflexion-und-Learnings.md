# Reflexion und Learnings <img width="28" height="24" alt="image" src="https://github.com/user-attachments/assets/2c048514-f14a-4e13-87ce-b22b65cd041c" />

## Ziel
### Was war das Ziel?
Unser Ziel war es, eine benutzerfreundliche Koffeinrechner-App zu entwickeln, Die Nutzer:innen hilft, ihren Koffeinkonsum besser zu verstehen und zu kontrollieren. Durch zusätzliche Funktionen wie Tipps, Alternativen, Challenges und die Möglichkeit, Freunde hinzuzufügen, sollte die App motivierend und alltagstauglich gestaltet werden.

### Wurde das Ziel erreicht?
Das Ziel wurde grösstenteils erreicht, da die App als übersichtlich, hilfreich und einfach in der Nutzung wahrgenommen wird. Besonders die klare Struktur und die motivierenden Zusatzfunktionen tragen dazu bei, dass Nutzer:innen die App positiv bewerten und grundsätzlich weiterempfehlen würden.

## Planung
### Wie haben wir uns organisiert?
Wir haben uns im Team klar aufgeteilt un die Aufagben nach Bereichen wie Design, Funktionen und Tests verteilt. Durch regelmässigen Austausch konnten wie den Fortschritt koordinieren und Probleme früh erkennen.

### Zeitmanagement?
Insgesamt haben wir unsere Zeit sehr sinnvoll und gut eingeplant. Wir sind immer gemäss den Angaben für KW06-08 vorgegangen welche wir von unseren Dozenten bekommen haben.

### Technische Umsetzung
Die App wurde von uns schrittweise aufgebaut und weiterentwickelt. Beginnend mit den Grundfunktionen wie Login oder persönlichen Angaben und anschliessend erweiterert mit Koffeinrechner, Tipps Challenges etc. Dabei wurde darauf geachtet, die Bedienung möglichst einfach zu halten.

### Was gab es für Herausforderungen?
Die grösste Herausforderung war die sinnvolle Strukturierung der Funktionen welche wir aber als Team super gemeistert haben.

### Wie sind wir damit umgegangen?
Probleme wurden gemeinsam besprochen und anschliessend nach Lösungen gesucht, wodurch wir effizient weiterarbeiten konnten.

## Learnings
Was haben wir gelernt? Wir durften  

Wie lief unsere Zusammenarbeit? Unsere Zusammenarbeit lief hervorragend. Die Aufgabenteilung und Kommunikation verlief reibungslos. Ausserdem haben wir für alle Probleme stets eine Lösung als Team gefunden und uns gegenseitig unterstützt. Wir konnten uns stets aufeinander verlassen.

Was hätte besser laufen können?
